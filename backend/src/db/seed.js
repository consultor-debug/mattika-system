...
// Aplica el schema (idempotente: CREATE TABLE IF NOT EXISTS ...).
async function applySchema(db = pool) {
  const schema = fs.readFileSync(path.join(__dirname, 'schema.sql'), 'utf8');
  await db.query(schema);
}

// Siembra tenants/usuarios/condiciones demo. Idempotente (ON CONFLICT).
// NO cierra el pool — se puede llamar desde el arranque del servidor.
...
module.exports = { applySchema, seedData };

// Ejecución directa por CLI (npm run seed)
if (require.main === module) {
  (async () => {
    await applySchema(pool);
    console.log('✅ Schema applied');
    await seedData(pool);
    console.log('✅ Datos demo sembrados. Credenciales:');
